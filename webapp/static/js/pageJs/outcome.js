    ...
    columns: [
        ...
        },
        {
            ...
            formatter: function (value, row, index) {
                ...
                if(row.orderStatus==0 && row.outcomeStatus==0){
                    return e + d + f+p;
                }else if(row.orderStatus==1 && row.outcomeStatus==0){
                    return e + d + f+t;
                }else{
                    return e + d + f
                }
...
function generateOrder(id) {
    $.post("/outComeItem/generateOrder/" + id,
        function (data) {
...
function orders(id) {
    ...
    $('#mytab2').bootstrapTable({
        method: 'post',
        contentType: "application/x-www-form-urlencoded",//必须要有！！！！
        url: "/outComeItem/outComeItemLists/"+id,//要请求数据的文件路径
