...
$('#mytab').bootstrapTable({
    ...
    url: "/hotelType/hotelTypeList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="hotelType"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(hotelTypeid, status) {
    if (status == 0) {
    ...
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/hotelType/deleteHotelType/' + hotelTypeid,
            dataType: 'json',
...
function edit(name) {
    $.post("/hotelType/findHotelType/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/hotelType/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/hotelType/hotelTypeList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/hotelType/hotelTypeList'});
}
...
$("#updateSta").click(function () {
    layer.confirm('确认要执行批量修改收支房型状态吗？',function(index){
        $.post(
            "/hotelType/deleteManyHotelType",
            {
                "manyId":$("#statusId").val(),
