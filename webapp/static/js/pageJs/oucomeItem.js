...
$('#mytab').bootstrapTable({
    ...
    url: "/oucomeItem/oucomeItemList",//要请求数据的文件路径
    ...
    columns: [
        ...
        },
        {
            ...
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="oucomeItem"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable(
        'refresh',
        {
            url: '/oucomeItem/findOucomeItemList',
            query: {
                id: $("#id__").val(),
                outcomeId: $("#outcomeId__").val(),
                subjectName: $("#subjectName__").val(),
                time: $("#time__").val(),
                money: $("#money__").val(),
            }
        }
...
function del(id, status) {
    ...
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/oucomeItem/deleteOucomeItem/' + id,
            dataType: 'json',
...
function edit(name) {
    $.post("/oucomeItem/findOucomeItem/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/oucomeItem/updateStatus/" + id + "/" + status,
        function (data) {
            if (status == 0) {
                if (data.message == "ok") {
...
//查询按钮事件
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/oucomeItem/oucomeItemList'});
})
...
$("#update").click(function () {
    $.post(
        "/oucomeItem/oucomeItemUpdateSave",
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/oucomeItem/deleteManyOucomeItem",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
