...
$('#mytab').bootstrapTable({
    ...
    url: "/inComeItem/inComeItemList",//要请求数据的文件路径
    ...
    columns: [
        ...
        },
        {
            ...
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="inComeItem"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable(
        'refresh',
        {
            url: '/inComeItem/findInComeItemList',
            query: {
                id: $("#id__").val(),
                incomeId: $("#incomeId__").val(),
...
function del(id, status) {
    ...
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/inComeItem/deleteInComeItem/' + id,
            dataType: 'json',
...
function edit(name) {
    $.post("/inComeItem/findInComeItem/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/inComeItem/updateStatus/" + id + "/" + status,
        function (data) {
            if (status == 0) {
                if (data.message == "ok") {
...
//查询按钮事件
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/inComeItem/inComeItemList'});
})
...
$("#update").click(function () {
    $.post(
        "/inComeItem/inComeItemUpdateSave",
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/inComeItem/deleteManyInComeItem",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
